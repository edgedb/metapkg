...
class MacOSAddUserAction(targets.TargetAction):
    ...
    def get_script(self, *, name, group=None, homedir=None,
        ...
        if group:
            ...
            if description:
                groupadd_cmds.append(
                    self._get_dscl_cmd(groupname, key='RealName',
                                       value=description)
                )
        ...
        useradd_cmds.append(
            self._get_dscl_cmd(username))

        if homedir:
            useradd_cmds.append(
                self._get_dscl_cmd(username,
                                   key='NFSHomeDirectory', value=homedir))
        if shell:
            useradd_cmds.append(
                self._get_dscl_cmd(username,
            ...
            useradd_cmds.append(
                self._get_dscl_cmd(username,
                                   key='UserShell', value='/sbin/nologin'))
        if description:
            useradd_cmds.append(
                self._get_dscl_cmd(username,
                                   key='RealName', value=description))
        ...
        useradd_cmds.append(
            self._get_dscl_cmd(username,
                               key='IsHidden', value='1'))

        last_uid = self._get_dscl_cmd(
            '/Users', action='list', key='UniqueID')
    ...
    def _get_dscl_cmd(self, name, *, action='create', key=None,
                      value=None, indent=0):
        args = {
            '.': None,
            action: None,
